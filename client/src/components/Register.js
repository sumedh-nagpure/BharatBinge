...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (age < 18 || age > 100) {
      swal("Invalid age", "Age must be between 18 and 100", "warning");
      return;
    }
    if (!email) {
    ...
    try {
      ...
      if (response.status === 200) {
        swal("Account Created", "Login to access your account.", "success").then((value) => {
          if (value) {
            navigate("/login");
          }
        });
      ...
      if (
        ...
        swal(error.response.data.swal_title, error.response.data.swal_message, "error");
      } else {
    ...
    <div className="h-screen">
      <div className="flex brightness-25 loginDiv h-full bg-center bg-cover bg-no-repeat bg-[url('../../src/BB.png')] justify-center">
        ...
        >
          ...
          <span className="text-white text-start mt-5 my-3 block text-4xl">Create Account</span>
          <div className="my-5">
            <input
          ...
          <div className="my-5">
            ...
            />
            {ageError && <span className="text-red-500">{ageError}</span>}
          </div>
          <div className="my-5">
            <input
              placeholder="Email"
          ...
          <button type="submit" class="border-2 text-white text-xl bg-black rounded py-2 px-6 border-[#E50914]">
            Register
          </button>
